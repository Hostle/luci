...
local libcbox2 = {
	bufferFile = "/tmp/queue";
	logFile = "/tmp/cbox.log";
	cmd = "";
	portName = "";
	port = "";
	verbose = 1;
...
function libcbox2.openPort(self,portName,Serial, verbose) 
local Serial = require("luars232")
local ok = 0;
local failed = 1;
...
function libcbox2.readPort(self,port,read_len, timeout, forced)
local port = self.port;
local portName = self.portName;
...
local forced = tonumber(forced);
local ok = 0;
local failed = 1;

	if(verbose > 0) then 
		print(string.format("<== READING ( %d )bytes FROM PORT [%s] WITH TIMEOUT ( %d )ms" ,read_len, portName, timeout)) 
	...
	if(data) then
		...
		if(verbose > 0) then 
			...
			print(string.format(printColor("==> DATA RECEIVED  %s ", 'yellow'), printColor(tostring(data), 'green')));
			end;
	...
	if(verbose > 0) then 
		print(string.format("TRANSMISSION FAILED ( %d/%d bytes ) READ FROM PORT [%s] \n" ,read_len or 1, size or 1, portName))
	end;
	port:flush();
	return tonumber(failed), error;
end
...
function libcbox2.newMsg(some_data)
  fifo(
    function(queue)
...
function libcbox2.getConfig(self, conf, sec)
